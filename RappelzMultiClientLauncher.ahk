...
global UpdateURL := "https://raw.githubusercontent.com/Joshmccoy14/Rappelz-Automation-Nexus/main/"
BaseDir := A_ScriptDir
...
DownloadUpdate(NewVersion) {
    global BaseDir
    Progress, 0, Downloading updates..., Updating
    
    ; Create folders
    FileCreateDir, %BaseDir%\backups
    FileCreateDir, %BaseDir%\backups\Launcher
    FileCreateDir, %BaseDir%\Launcher
    Loop, 8 {
        FileCreateDir, %BaseDir%\backups\Launcher\win%A_Index%
        FileCreateDir, %BaseDir%\Launcher\win%A_Index%
    }
    
    ; Main files
    DownloadFile("RappelzMultiClientLauncher.ahk", "RappelzMultiClientLauncher.ahk")
    DownloadFile("findtext2.ahk", "findtext2.ahk")
    DownloadFile("changelog.txt", "changelog.txt")
    
    ; Win1-8 folders
    Loop, 8 {
        DownloadFile("Launcher/win" . A_Index . "/Rappelz%20Automation%20Nexus.ahk", "Launcher\win" . A_Index . "\Rappelz Automation Nexus.ahk")
        DownloadFile("Launcher/win" . A_Index . "/findtext2.ahk", "Launcher\win" . A_Index . "\findtext2.ahk")
    }
    
    ; Update version
    global UpdateURL
    VersionURL := UpdateURL . "version.txt"
    Content := HttpGet(VersionURL)
    FileDelete, version.txt
    FileAppend, %Content%, version.txt
    
    Progress, Off
    MsgBox, 64, Update Complete, Update installed successfully!`n`nRestart the launcher.
}

DownloadFile(URLPath, LocalPath) {
    global UpdateURL, BaseDir
    if FileExist(BaseDir . "\" . LocalPath)
        FileCopy, %BaseDir%\%LocalPath%, %BaseDir%\backups\%LocalPath%, 1
    FullURL := UpdateURL . URLPath
    Content := HttpGet(FullURL)
    FileDelete, %BaseDir%\%LocalPath%
    FileAppend, %Content%, %BaseDir%\%LocalPath%
}
