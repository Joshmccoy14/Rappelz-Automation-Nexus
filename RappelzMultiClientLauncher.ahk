...
global UpdateURL := "https://example.com/launcher/version.txt"  ; Change to your update server
global DownloadURL := "https://example.com/launcher/update.zip"  ; Change to your update server
...
ShowMainGui() {
    ...
    Gui, Add, Button, x310 y450 w150 h25 gCheckUpdate, Update
    ...
    Gui, Add, Button, x20 y480 w440 h25 gAbout, About / Version Info
...
CheckUpdate:
    global CurrentVersion, UpdateURL, DownloadURL
    
    MsgBox, 4, Update Check, Checking for updates...`n`nCurrent Version: %CurrentVersion%`n`nDo you want to check for updates?
    IfMsgBox No
        Return
    
    ; Download version info
    tempFile := A_Temp . "\launcher_version.txt"
    UrlDownloadToFile, %UpdateURL%, %tempFile%
    
    if (ErrorLevel) {
        MsgBox, 16, Update Error, Failed to check for updates.`n`nPlease check your internet connection or try again later.
        Return
    }
    ...
    if (remoteVersion = "") {
        MsgBox, 16, Update Error, Failed to read version information.
        Return
    }
    
    ; Compare versions
    if (CompareVersions(remoteVersion, CurrentVersion) > 0) {
        MsgBox, 4, Update Available, A new version is available!`n`nCurrent Version: %CurrentVersion%`nNew Version: %remoteVersion%`n`nDo you want to download and install the update?
        IfMsgBox Yes
        {
            Gosub, DownloadUpdate
        }
    } else {
        MsgBox, 64, Up to Date, You are running the latest version!`n`nVersion: %CurrentVersion%
    }
Return
...
DownloadUpdate:
    ...
    updateFile := A_Temp . "\launcher_update.zip"
    
    ; Show progress
    Progress, b w300, Downloading update..., Please wait, Updating Launcher
    
    UrlDownloadToFile, %DownloadURL%, %updateFile%
    
    Progress, Off
    
    if (ErrorLevel) {
        MsgBox, 16, Download Error, Failed to download update.`n`nPlease try again later.
        Return
    }
    
    MsgBox, 4, Update Downloaded, Update downloaded successfully!`n`nThe launcher will now extract and apply the update.`n`nContinue?
    IfMsgBox No
    {
    ...
    updateScript := A_ScriptDir . "\update_launcher.bat"
    FileDelete, %updateScript%
    
    FileAppend, @echo off`n, %updateScript%
    FileAppend, timeout /t 2 /nobreak > nul`n, %updateScript%
    FileAppend, echo Extracting update...`n, %updateScript%
    FileAppend, powershell -command "Expand-Archive -Path '%updateFile%' -DestinationPath '%A_ScriptDir%' -Force"`n, %updateScript%
    FileAppend, del "%updateFile%"`n, %updateScript%
    FileAppend, echo Update complete!`n, %updateScript%
    FileAppend, timeout /t 2 /nobreak > nul`n, %updateScript%
    FileAppend, start "" "%A_ScriptName%"`n, %updateScript%
    FileAppend, del "`%`%~f0"`n, %updateScript%
...
CompareVersions(v1, v2) {
    ...
    Loop, 3 {
        p1 := (A_Index <= v1Parts0) ? v1Parts%A_Index% : 0
        p2 := (A_Index <= v2Parts0) ? v2Parts%A_Index% : 0
        p1 += 0
        p2 += 0
...
About:
    ...
    MsgBox, 64, About, Rappelz Multi-Client Launcher`n`nVersion: %CurrentVersion%`n`nA tool for managing multiple Rappelz game clients.`n`nFeatures:`n- Launch multiple clients`n- Auto-login support`n- Thumbnail view`n- Auto-update capability
Return
